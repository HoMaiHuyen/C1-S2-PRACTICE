...
function getCartTotalAmount() {
    ...
    SHOPPING_CART.forEach(pro => {
        const product = PRODUCTS.find(product => product.id === pro.id);
        if (product) {
            result += product.price * pro.quantity;
        }
    })
  return result;
}
...
function addProductToCart(productId) {
    // Write your code here
     let existProduct = SHOPPING_CART.find(pro => pro.id === productId);
    if (existProduct) {
...
function removeProductFromCart(productId) {
    // Write your code here
    let existProductIndex = SHOPPING_CART.findIndex(pro => pro.id === productId);
    if (existProductIndex !== -1) {
        let existProduct = SHOPPING_CART[existProductIndex];
        if (existProduct.quantity > 1) {
            existProduct.quantity--;
        } else {
            SHOPPING_CART.splice(existProductIndex, 1);
        }
    }
}
